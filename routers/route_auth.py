...
from schemas import Csrf, UserBody, UserInfo, SuccessMsg
...
async def signin(request: Request, response: Response, user: UserBody, csrf_protect: CsrfProtect = Depends()):
    csrf_token = csrf_protect.get_csrf_from_headers(request.headers)
    csrf_protect.validate_csrf(csrf_token)
    user = jsonable_encoder(user)
...
def get_user_refresh_info(request: Request, response: Response):
